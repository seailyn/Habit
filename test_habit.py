...
class TestHabit:
    ...
    def test_create_habit(self):
        habit_data = Habit(habit_id = 999, name = "name", description = "description", period = "daily")
        habit_data.add_habit()

        test_habit = db.get_habit_by_id(habit_data.habit_id)
    ...
    def test_complete_date(self):
        ...
        test_date_1 = db.check_date(999, completed_date)
        db.mark_complete(999, completed_date)

        test_date_2 = db.check_date(999, completed_date)
        assert test_date_1 == False
        assert test_date_2 == True


    def test_incomplete_date(self):
        incompleted_date = date.today()
        test_date_1 = db.check_date(999, incompleted_date)
        db.mark_incomplete(999, incompleted_date)

        test_date_2 = db.check_date(999, incompleted_date)
        assert test_date_1 == True
        assert test_date_2 == False
    ...
    def test_edit_habit(self):
        edit_habit_data = Habit(habit_id = 999, name = "edited_name", description = "edited_description", period = "edited")
        edit_habit_data.edit_habit()

        test_habit = db.get_habit_by_id(edit_habit_data.habit_id)
    ...
    def test_delete_habit(self):
        ...
        test_habit = db.check_for_habit_by_id(delete_habit_data.habit_id)
        assert test_habit == False
    ...
    def test_date_count(self):
        ...
        for i in range(1, 5):
            count = db.get_habit_completion_count(i)

...
def main_check():
    ...
    """
    print("Please select the habit you want to complete.")
    habits = questionary.select(
        "These are your current habits:",
        choices=analytics.return_questionary_choice_habits(),
    ).ask()
    ...
    if today:
        ...
        if analytics.check_date(habit_id, date_today):
            uncheck_today = questionary.confirm(
                f"Habit has already been checked for {date_today}. Do you want to uncheck {date_today}?"
            ).ask()
    ...
    if not today:
        entered_date = questionary.text(
            "Please enter the date(YYYY-MM-DD) you want to check"
        ).ask()
        ...
        if approval:
            uncheck_date = questionary.confirm(
                f"Habit has already been checked for {entered_date}. Do you want to uncheck?"
            ).ask()
...
def main_edit():
    ...
    name = questionary.text(
        f"Please enter the new habit name(current: {old_habit.name}):"
    ).ask()
    description = questionary.text(
        f"Please enter the new habit description:(current: {old_habit.description}):"
    ).ask()
    period = questionary.select(
        "In what periodicity should your Habit be tracked?",
        choices=["daily", "weekly", "monthly", "yearly"],
...
def main_delete():
    ...
    """
    print("Please select the habit you want to delete.")
    habits = questionary.select(
        "These are your current habits:",
        choices=analytics.return_questionary_choice_habits(),
    ).ask()
    ...
    approval = questionary.confirm(
        f"Are you sure you want to delete Habit: {habit.name}?"
    ).ask()
    if approval:
        habit.delete_habit()
...
def main_analyze():
    ...
    """
    print("Please select the habit you want to analyze.")
    habits = questionary.select(
        "These are your current habits:",
        choices=analytics.return_questionary_choice_habits(),
    ).ask()
    ...
    print(
        f"Habit ID:               {habit_data.habit_id}\n"
...
def main_stats():
    ...
    """
    stats = questionary.select(
        "Please enter the habit tracking information:",
        choices=[
    ...
    if stats == "List of daily habits":
        ...
        print(
            f"Your overall longest streak is {analytics.return_overall_longest_streak()}."
        )
